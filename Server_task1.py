...
def token_scaduto(user):
    ...
    for tk in token:
        if tk['username'] == user and (time.time() - tk['data']) > 1800: 
            return True
        else:
            return False
    return False
    ...
    if request.args['username'] in lista_utenti.keys():
        ...
        if int(request.args['password']) == lista_utenti[request.args['username']]:
            ...
            if token_scaduto(request.args['username']):
                #aggiorna il toke se è scaduto
                response=aggiorna_token(token)
                return jsonify(response)

            elif missing_token(request.args['username']):
                #crea il token per la prima volta
                response=crea_nuovo_token(token)
                return jsonify(response)

            else:
                #verifica se il toke è ancora valido
                response=controlla_token(token)
                return jsonify(response)
...
def send():
    ...
    if request.args['destinatario'] not in lista_utenti.keys():
        ...
        response=nuovo_messaggio(messaggi)
        return jsonify(response)
